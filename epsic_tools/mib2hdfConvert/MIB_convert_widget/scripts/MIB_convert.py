...
class convert_info_widget():
    ...
    def _organize(self, year, session, subfolder, no_reshaping, 
        ...
        print("destination_path: "+dest_path)
        ...
        print("script_save_path: "+script_save_path)
        if not os.path.exists(script_save_path):
            os.makedirs(script_save_path)
            print("created: "+script_save_path)
...
#SBATCH --mem 100G
'''
f"#SBATCH --array=0-{len(to_convert)-1}%3\n"
f"#SBATCH --error={script_save_path}{os.sep}error_%j.out\n"
f"#SBATCH --output={script_save_path}{os.sep}output_%j.out\n"
'''
...
f"python {python_script_path} {info_path} $SLURM_ARRAY_TASK_ID\n"
        ...
        if create_info_check:
            ...
            with open (info_path, 'w') as f:
                f.write(
                    f"to_convert_paths = {to_convert}\n"
                    f"proc_path = /dls/e02/data/{year}/{session}/processing/Merlin/{subfolder}\n"
                    f"src_path = /dls/e02/data/{year}/{session}/Merlin/{subfolder}\n"
